...
def tokenize_function(examples):
    # Apply memoized single text tokenization to each example
    # Ensure that `input_ids`, `token_type_ids`, `attention_mask` are lists of lists
    # as expected by the Dataset.from_dict if you were to reconstruct
    # However, for direct use with `map`, returning a dict of lists of tensors or integer lists is fine.
    # The current approach with `[0]` index is incorrect as it takes only the first item if batching.
    # It should apply tokenization to each text in the 'examples['text']' list.
...
def train_model(train_dataset, eval_dataset, label_encoder, num_epochs: int = 3, learning_rate: float = 5e-5, output_dir: str = "./results"):
    ...
    return model, label_encoder  # Only return what's needed# ========== STEP 4: LOAD THE TRAINED MODEL ==========
...
if __name__ == "__main__":
    ...
    initial_df.dropna(subset=['text', 'label'], inplace=True) # Ensure 'text' is also checked
    ...
    hf_df = load_additional_huggingface_datasets() # This call will benefit from memoization

    # Robust NaN handling for hf_df
    if not hf_df.empty:
        hf_df['label'] = hf_df['label'].replace(['nan', 'Nan', 'NAN', 'None'], np.nan)
        hf_df.dropna(subset=['text', 'label'], inplace=True) # Ensure 'text' is also checked
    ...
    final_dataset_df.dropna(subset=['text', 'label'], inplace=True) # Ensure 'text' is also checked
    ...
    # Check if there's enough data after all processing
    if final_dataset_df.empty or len(final_dataset_df['label'].unique()) < 2:
        print("Insufficient data or labels after processing all sources. Exiting.")
        exit()
    ...
    # The map function calls tokenize_function for each batch.
    # The memoization is applied at the _tokenize_single_text level,
    # so repeated texts within a batch or across batches will hit the cache.
    ...
    # Initialize model and label_encoder to None or default values
    model = None
    tokenizer_loaded = None # Renamed to avoid shadowing global tokenizer
    label_encoder_final = None # Renamed to avoid shadowing

    # Determine if we need to train a new model or load an existing one
    ...
    if os.path.exists(MODEL_SAVE_DIR) and os.path.exists(os.path.join(MODEL_SAVE_DIR, "label_map.json")):
        temp_model, temp_tokenizer, temp_label_encoder = load_model(MODEL_SAVE_DIR)
        if temp_model is not None:
            model = temp_model
            tokenizer_loaded = temp_tokenizer
            label_encoder_final = temp_label_encoder
            print("Existing model loaded successfully.")
            # Set global tokenizer to the loaded one if successful
            globals()['tokenizer'] = tokenizer_loaded
            should_train_new_model = False
        else:
            print("Existing model could not be loaded. Will retrain from scratch.")

    if should_train_new_model:
        print("Training a new model.")
        model, label_encoder_final = train_model(tokenized_train_dataset, tokenized_eval_dataset, label_encoder)
    
    # Use the finalized label_encoder for keyword generation and classification
    # If a model was loaded, label_encoder_final comes from load_model
    # If a model was trained, label_encoder_final comes from train_model
    if label_encoder_final is None:
        print("Error: label_encoder is not initialized. Exiting.")
        exit()
    ...
    df_for_keywords.dropna(subset=['text', 'label'], inplace=True) # Ensure 'text' is also checked
    ...
    df_for_keywords['label'] = df_for_keywords['label'].astype(str).str.strip().str.title().replace('Math', 'Mathematics')

    # Ensure all labels in df_for_keywords are known by the label_encoder
    # Filter out labels not seen by the encoder during fit_transform
    known_labels = set(label_encoder_final.classes_)
    df_for_keywords = df_for_keywords[df_for_keywords['label'].isin(known_labels)].copy()


    updated_labels_encoded = label_encoder_final.transform(df_for_keywords['label'].tolist())
    updated_raw_dataset = Dataset.from_dict({'text': df_for_keywords['text'].tolist(), 'label': updated_labels_encoded.tolist()})

    dynamic_keywords = generate_keywords_from_dataset(updated_raw_dataset, label_encoder_final)
    print("\nDynamic keywords generated:")
    for subject, keywords in dynamic_keywords.items():
        print(f"  {subject}: {keywords}")

    classifier_pipeline = None
    if model is not None and tokenizer_loaded is not None: # Use tokenizer_loaded if model was loaded
        try:
            classifier_pipeline = pipeline(
                "text-classification",
                model=model, # Pass the loaded/trained model directly
                tokenizer=tokenizer_loaded if tokenizer_loaded else tokenizer, # Use the loaded tokenizer, or the global one
        ...
        print("\nSkipping pipeline loading as model or tokenizer are not available.")
    ...
    if classifier_pipeline:
        ...
        while True:
            ...
            if user_input.lower() == 'quit':
                ...
                # Clear memoization caches before retraining to ensure fresh data is loaded/processed
                load_data_from_file.cache_clear()
                load_additional_huggingface_datasets.cache_clear()
                _tokenize_single_text.cache_clear()
                load_model.cache_clear() # Invalidate model cache as model will be retrained

                # Need to re-prepare the dataset completely to get updated train/eval splits
                # after new data is potentially merged
                _, _, _, combined_df_after_retrain_prep = retrain_with_new_data(current_dataset_path, new_samples_base_dir, retrain_trigger=True)

                if combined_df_after_retrain_prep is not None:
                    # Re-fit label encoder and prepare datasets from the new combined_df
                    labels_retrain = combined_df_after_retrain_prep['label'].tolist()
                    label_encoder_final = LabelEncoder() # Re-initialize label encoder
                    encoded_labels_retrain = label_encoder_final.fit_transform(labels_retrain)

                    retrain_dataset_dict = {'text': combined_df_after_retrain_prep['text'].tolist(), 'label': encoded_labels_retrain.tolist()}
                    retrain_raw_dataset = Dataset.from_dict(retrain_dataset_dict)

                    unique_labels_retrain, counts_retrain = np.unique(encoded_labels_retrain, return_counts=True)
                    can_stratify_retrain = all(c >= 2 for c in counts_retrain)

                    if can_stratify_retrain:
                        train_texts_retrain, val_texts_retrain, train_labels_retrain, val_labels_retrain = train_test_split(
                            retrain_raw_dataset['text'], retrain_raw_dataset['label'], test_size=0.1, random_state=42, stratify=retrain_raw_dataset['label']
                        )
                    else:
                        print("Warning: Cannot use stratified split during retraining due to very small classes. Performing non-stratified split.")
                        train_texts_retrain, val_texts_retrain, train_labels_retrain, val_labels_retrain = train_test_split(
                            retrain_raw_dataset['text'], retrain_raw_dataset['label'], test_size=0.1, random_state=42
                        )
                    
                    train_hf_dataset_retrain = Dataset.from_dict({'text': train_texts_retrain, 'label': train_labels_retrain})
                    eval_hf_dataset_retrain = Dataset.from_dict({'text': val_texts_retrain, 'label': val_labels_retrain})

                    tokenized_train_dataset_retrain = train_hf_dataset_retrain.map(tokenize_function, batched=True)
                    tokenized_eval_dataset_retrain = eval_hf_dataset_retrain.map(tokenize_function, batched=True)

                    model, label_encoder_final = train_model(tokenized_train_dataset_retrain, tokenized_eval_dataset_retrain, label_encoder_final)

                    # Reload the pipeline with the newly trained model
                    if model is not None:
                        try:
                            classifier_pipeline = pipeline(
                                "text-classification",
                                model=MODEL_SAVE_DIR, # Reload from saved path
                                tokenizer=MODEL_SAVE_DIR, # Reload from saved path
                                device=0 if torch.cuda.is_available() else -1
                            )
                            print("\nHugging Face classification pipeline reloaded after retraining.")
                        except Exception as e:
                            print(f"\nCould not reload pipeline after retraining: {e}")
                            classifier_pipeline = None # Invalidate pipeline if reload fails
                    else:
                        classifier_pipeline = None # Invalidate pipeline if retraining failed

                else:
                    print("Retraining process did not yield a valid dataset. Skipping model retraining.")
                ...
                updated_df_for_keywords.dropna(subset=['text', 'label'], inplace=True) # Ensure 'text' is also checked
                ...
                # Ensure all labels in updated_df_for_keywords are known by the label_encoder
                if label_encoder_final is not None:
                    known_labels_retrain = set(label_encoder_final.classes_)
                    updated_df_for_keywords = updated_df_for_keywords[updated_df_for_keywords['label'].isin(known_labels_retrain)].copy()
                    if not updated_df_for_keywords.empty:
                        updated_labels_encoded = label_encoder_final.transform(updated_df_for_keywords['label'].tolist())
                        ...
                        print("No valid data for keyword regeneration after retraining.")
                        dynamic_keywords = {} # Reset keywords if no data
                else:
                    print("Label encoder is not available after retraining. Cannot regenerate dynamic keywords.")
                    dynamic_keywords = {}
                continue
