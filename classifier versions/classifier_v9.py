...
def combine_and_limit_datasets(local_df, hf_df, english_limit=2000, other_limit=3000, local_priority_for_other=1500):
    ...
    Args:
        ...
        other_limit (int): Max samples for all other classes (total per class).
        local_priority_for_other (int): Max samples to take from local_df for 'other' classes
    ...
    for label in all_labels:
        ...
        if label == "English":
            # For English, combine first and then sample
            ...
            # For other labels, prioritize local, then supplement from HF
            current_samples_for_label = []

            # Take from local dataset first
            ...
            # Supplement from Hugging Face if needed
            if remaining_needed > 0:
                # Exclude samples already taken from local to avoid duplicates if they exist in HF too
                # This simple approach assumes no exact text duplicates between local and HF.
                # For robust de-duplication, you might need a more complex merge/deduplicate step.
...
def main():
    ...
    # Combine datasets with specific sampling logic
